...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Reduce padding to fix the "space between image scroll" issue
    this.padding = 1; // Was 5
...
export default function FlyingPosters({
  ...
  useEffect(() => {
    ...
    // We MUST destroy the old instance if it exists to prevent memory leaks and overlapping renders
  ...
  useEffect(() => {
    ...
    const handleWheel = e => {
      // Don't prevent default on the canvas itself, let the page scroll naturally
      // but still update the poster scroll
      if (instanceRef.current) {
    ...
    // Use passive: true so it doesn't block the natural page scroll
