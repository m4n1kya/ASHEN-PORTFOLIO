...
const ProjectsWindow = ({ onBack, initialProject = "ashenritual" }) => {
  ...
  return (
    <div className="projects-window fixed inset-0 z-[1000] bg-black flex flex-col w-full h-full overflow-hidden">
      ...
      {/* Fixed Content Overlay (Sidebar on Desktop, Bottom Sheet on Mobile) */}
      <div className="absolute inset-0 top-32 z-10 pointer-events-none flex flex-col md:flex-row p-4 md:p-8 xl:p-16">
        
        {/* Glass Panel Container */}
        <div className="w-full md:w-[450px] lg:w-[500px] xl:w-[600px] h-full flex flex-col justify-end md:justify-center relative">
          ...
          {projectsData.map(project => {
            ...
            return (
              <div 
                key={project.id}
                className={`absolute bottom-0 md:bottom-auto w-full bg-black/40 backdrop-blur-2xl border border-white-50/10 rounded-3xl p-6 md:p-8 lg:p-10 pointer-events-auto transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col gap-5 md:gap-6 shadow-2xl ${isActive ? 'opacity-100 translate-y-0 scale-100 z-10' : 'opacity-0 translate-y-12 scale-95 -z-10 pointer-events-none'}`}
              >
                  <h3 className="text-2xl md:text-3xl lg:text-4xl font-bold text-white tracking-widest uppercase leading-tight drop-shadow-lg">{project.name}</h3>
                  <p className="text-sm md:text-base leading-relaxed text-white-50/90 line-clamp-3 md:line-clamp-4">
                      {project.shortDescription}
                  </p>
                  
                  {/* Markdown Content (Clamped so it fits in the fixed window) */}
                  <div className="hidden sm:block prose prose-sm md:prose-base prose-invert max-w-none border-t border-white-50/10 pt-5">
                     <div className="line-clamp-4 md:line-clamp-6 text-white-50/80">
                  ...
                  <div className="flex flex-wrap gap-3 md:gap-4 mt-2">
                    {project.liveUrl && (
                      <a href={project.liveUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-2.5 bg-white text-black text-sm font-bold rounded-lg hover:bg-white-50 transition-all shadow-[0_0_20px_rgba(255,255,255,0.15)] hover:shadow-[0_0_25px_rgba(255,255,255,0.3)] hover:-translate-y-0.5">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                        Live Demo
                      </a>
                    )}
                    <a href={project.repoUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-2.5 bg-black-300/80 backdrop-blur-md border border-white-50/20 text-white text-sm font-bold rounded-lg hover:bg-white-50/10 transition-all hover:-translate-y-0.5">
